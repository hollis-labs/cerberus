	...
	return contract.Finalize(contract.Definition{
		...
		Operations: []contract.Operation{
			...
			},
			{
				...
				Cost:        contract.CostNone,
				LocalFS:     contract.LocalFSNone,
				Description: "Read a site's deployment script.",
				InputSchema: contract.ObjectSchema(map[string]any{"server_id": contract.IntegerSchema("Forge server ID."), "site_id": contract.IntegerSchema("Forge site ID.")}, "server_id", "site_id"),
			},
			{
			...
			},
			{
				...
				Examples:    []string{"cerberus forge deploy 12 34 --dry-run", "cerberus forge deploy 12 34 --ack"},
				InputSchema: contract.ObjectSchema(map[string]any{"server_id": contract.IntegerSchema("Forge server ID."), "site_id": contract.IntegerSchema("Forge site ID.")}, "server_id", "site_id"),
			},
			{
				...
				Examples:    []string{"cerberus forge exec 12 34 'php artisan migrate --force' --dry-run", "cerberus forge exec 12 34 'php artisan migrate --force' --ack"},
				InputSchema: contract.ObjectSchema(map[string]any{"server_id": contract.IntegerSchema("Forge server ID."), "site_id": contract.IntegerSchema("Forge site ID."), "command": contract.StringSchema("Command to execute.")}, "server_id", "site_id", "command"),
			},
		},
	})
}
