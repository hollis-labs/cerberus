...
func Definition() contract.Definition {
	return contract.Definition{
		...
		Operations: []contract.Operation{
			...
			{Name: "update_deployment_script", Description: "Update a site's deployment script. Destructive: the script is what deploy_site runs next.", Examples: []string{"cerberus forge set-script 12 34 \"$(cat deploy.sh)\" --ack"}, InputSchema: contract.ObjectSchema(map[string]any{"server_id": contract.IntegerSchema("Forge server ID."), "site_id": contract.IntegerSchema("Forge site ID."), "content": contract.StringSchema("Deployment script content."), "auto_source": map[string]any{"type": "boolean", "description": "Automatically source environment variables."}}, "server_id", "site_id", "content"), Destructive: true},
