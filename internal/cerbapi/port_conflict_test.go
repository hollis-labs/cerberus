...
func TestPortConflictRefusesActivationBeforeBuildOrRuntimeMutation(t *testing.T) {
	...
	operations := []func(context.Context, string) (*OpResult, error){svc.ApplyResource, svc.ReloadResource, func(ctx context.Context, id string) (*OpResult, error) { return svc.DeployResource(ctx, id) }}
	for _, operation := range operations {
		result, err := operation(context.Background(), "first")
