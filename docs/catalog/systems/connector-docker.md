...
summary: "Runs container and Compose operations against a Docker daemon named per call — local, a docker context, or a remote host over ssh://."
state_field: "maturity"
state_label: "shipped"
review_status: "reviewed"
confidence_score: 0.9
confidence_label: "ps and logs verified live through the daemon; destroy is unreachable from any surface"
...
relationships:
  ...
  - type: "blocks"
    target: "CERB-GAP-271"
    note: "destroy is the only destructive operation with no dry-run"
  - type: "blocks"
    target: "CERB-GAP-272"
    note: "destroy is reachable from no surface"
---

# Docker connector

> Runs container and Compose operations against a Docker daemon named per call — local, a docker context, or a remote host over ssh://.
...
and `cerberus docker logs mtbf-monitor-app-1 --lines 3` both return real output.
...
from that call's config as either a `host` (a `DOCKER_HOST` value) or a
...
destructive and ack-gated — and has no dry-run preview, no CLI command and no
MCP tool. `cerberus docker down` and `cerberus_docker_down` both invoke `stop`.

## Owns

- Container list, start, stop, remove and logs via the docker CLI
- Compose up, down and ps against a compose file
- Per-call Docker target selection: DOCKER_HOST, a docker context, or neither
